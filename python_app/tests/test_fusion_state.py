...
class FusionStateTests(unittest.TestCase):
    ...
    def test_audio_dock_input_comes_from_serial_state(self) -> None:
        fusion = FusionState()
        serial_state = serial_with_wrist(pitch=0, roll=0)
        serial_state["devices"]["audiodock"] = {"input": "open workspace"}
        values = fusion.build_input_dict(serial_state, {}, now_s=0.0)

        self.assertEqual(values["audiodock_input"], "open workspace")
